...
namespace Multiplayer.Client
{
    public static class SyncMethods
    {
        ...
        static void StorageSettingsClipboardCopy_Prefix(ref bool __state)
        {
        ...
        // Finalizer, not postfix: a throw in Copy would leave dontSync stuck on,
        // silently disabling all syncing for the session
        [MpFinalizer(typeof(StorageSettingsClipboard), nameof(StorageSettingsClipboard.Copy))]
        static void StorageSettingsClipboardCopy_Finalizer(bool __state)
        {
            if (__state)
        ...
        static bool InitiateLoading_Prefix(IEnumerable<CompTransporter> transporters)
        {
            ...
            SyncedInitiateLoading(transporters.ToList());
            return false;
        }

        [SyncMethod]
        static void SyncedInitiateLoading(List<CompTransporter> transporters) =>
        ...
        {
            // Also during ticking: ideo reform can complete from ritual outcome toils
