...
namespace Multiplayer.Client
{
    ...
    public class MarkerInspectTab : InspectTabBase
    {
        ...
        private static void DrawMarkerRow(Rect rect, PingInfo info, LocationPings loc)
        {
            ...
            // Category icon. Untyped (Default or unresolved def) gets no icon - the placer-colour
            // stripe to the left already carries the row's identity.
            var iconRect = new Rect(rect.x + 12f, rect.y + 4f, 18f, 18f);
            var cat = info.category;
            var iconTex = cat?.IconTexture;
            if (iconTex != null)
            {
                using (MpStyle.Set(cat.tint))
        ...
        private static string HeaderLabel(int count)
            => "MpMarkerInspectTab_Header".Translate(count);
    }
}
