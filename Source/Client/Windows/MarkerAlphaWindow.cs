    ...
    public class MarkerAlphaWindow : Window
    {
        ...
        public override void DoWindowContents(Rect inRect)
        {
            ...
            using (MpStyle.Set(GameFont.Medium).Set(TextAnchor.UpperCenter))
                Widgets.Label(titleRect, "MpMarkerAlpha_Title".Translate());
            ...
            var pctLabel = "MpMarkerAlpha_Percent".Translate(Mathf.RoundToInt(alpha * 100f)).ToString();
            ...
            using (MpStyle.Set(GameFont.Tiny).Set(TextAnchor.UpperLeft).Set(ColoredText.SubtleGrayColor))
                Widgets.Label(minMaxRect,
                    "MpMarkerAlpha_MinHint".Translate(Mathf.RoundToInt(MinAlpha * 100f)));
            using (MpStyle.Set(GameFont.Tiny).Set(TextAnchor.UpperRight).Set(ColoredText.SubtleGrayColor))
                Widgets.Label(minMaxRect, "MpMarkerAlpha_MaxHint".Translate());
            TooltipHandler.TipRegion(sliderRect, "MpMarkerAlpha_MinTip".Translate());
            ...
            if (Widgets.ButtonText(cancelRect, "MpMarkerAlpha_Cancel".Translate()))
            {
                alpha = initialAlpha;
                ApplyToSelection();
                Close();
            }
            if (Widgets.ButtonText(applyRect, "MpMarkerAlpha_Apply".Translate()))
            {
