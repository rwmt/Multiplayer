    ...
    public class PingFiltersDialog : Window
    {
        ...
        public override void DoWindowContents(Rect inRect)
        {
            ...
            using (MpStyle.Set(GameFont.Medium).Set(TextAnchor.MiddleLeft))
                Widgets.Label(titleRect, "MpPingFilters_Title".Translate());

            var y = titleRect.yMax + 8f;

            var specRect = new Rect(inRect.x, y, inRect.width, 24f);
            var showSpec = settings.showSpectatorMarkers;
            Widgets.CheckboxLabeled(specRect, "MpPingFilters_ShowSpectators".Translate(), ref showSpec);
            ...
            var yy = 0f;
            DrawSectionHeader(new Rect(0f, yy, viewRect.width, SectionHeaderH), "MpPingFilters_FactionsHeader".Translate());
            yy += SectionHeaderH;

            if (factions.Count == 0)
            {
                DrawEmptyRow(new Rect(0f, yy, viewRect.width, EmptyRowH), "MpPingFilters_NoFactions".Translate());
                yy += EmptyRowH;
            }
            else
            {
            ...
            DrawSectionHeader(new Rect(0f, yy, viewRect.width, SectionHeaderH), "MpPingFilters_PlayersHeader".Translate());
            yy += SectionHeaderH;

            if (players.Count == 0)
            {
                DrawEmptyRow(new Rect(0f, yy, viewRect.width, EmptyRowH), "MpPingFilters_NoOtherPlayers".Translate());
                yy += EmptyRowH;
            }
            else
            {
            ...
            if (Widgets.ButtonText(resetRect, "MpPingFilters_ResetAll".Translate()))
            {
        ...
        private static void DrawPlayerRow(Rect rect, PlayerRowItem p, MpSettings s)
        {
            ...
            if (canShowClear)
            {
                TooltipHandler.TipRegion(clearRect, "MpPingFilters_ClearPlayerMarkers".Translate(new NamedArgument(p.username, "USERNAME")));
        ...
        private List<PlayerRowItem> ListOtherPlayers()
        {
            ...
            if (mePi != null)
                cachedOtherPlayers.Add(new PlayerRowItem(meName ?? "", (meName ?? "?") + " " + "MpPingFilters_YouSuffix".Translate(),
            ...
            if (comp != null)
            {
                foreach (var m in comp.AllMarkers)
                {
                    ...
                    others.Add(new PlayerRowItem(name, name + " " + "MpPingFilters_OfflineSuffix".Translate(), color));
                }
            }
        ...
        public static string OpenTooltipLabel()
            => "MpPingFilters_OpenTooltip".Translate();
    }
}
