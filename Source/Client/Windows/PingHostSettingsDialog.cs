...
public class PingHostSettingsDialog : Window
{
    ...
    public override void DoWindowContents(Rect inRect)
    {
        ...
        using (MpStyle.Set(GameFont.Medium).Set(TextAnchor.MiddleLeft))
            Widgets.Label(titleRect, MpTranslate.Fallback("MpPingHostSettings_Title", "Host settings"));
        ...
        if (comp != null)
        {
            ...
            var capLabel = MpTranslate.Fallback("MpPingMenuWindow_MarkerCapPerPlayer", "Marker limit per player");
            ...
            TooltipHandler.TipRegion(capRect, MpTranslate.Fallback("MpPingMenuWindow_MarkerCapPerPlayer_Tip",
                $"Maximum markers any single player can own. Range {PingMarkerCap.Min}-{PingMarkerCap.Max}. Excess markers are evicted oldest-first.",
                PingMarkerCap.Min, PingMarkerCap.Max));
            if (editCap != prevCap)
            {
        ...
        using (MpStyle.Set(GameFont.Tiny).Set(TextAnchor.MiddleLeft).Set(new Color(0.65f, 0.65f, 0.65f)))
            Widgets.Label(sectionHeaderRect, MpTranslate.Fallback("MpPingHostSettings_ClearAllHeader", "Clear everything for every player"));
        y = sectionHeaderRect.yMax + 4f;

        const float ButtonH = 28f;
        var clearMarkersRect = new Rect(inRect.x, y, inRect.width, ButtonH);
        if (Widgets.ButtonText(clearMarkersRect, MpTranslate.Fallback("MpPingHostSettings_ClearAllMarkers", "Clear all markers")))
        {
            Find.WindowStack.Add(Dialog_MessageBox.CreateConfirmation(
                MpTranslate.Fallback("MpPingHostSettings_ClearAllMarkers_Confirm",
                    "Remove every marker placed by every player? This cannot be undone."),
                () =>
                {
        ...
        if (Widgets.ButtonText(clearPingsRect, MpTranslate.Fallback("MpPingHostSettings_ClearAllPings", "Clear all pings")))
        {
    ...
    public static string OpenTooltipLabel()
        => MpTranslate.Fallback("MpPingHostSettings_OpenTooltip",
            "Host-only: set per-player marker limits and wipe all markers or pings.");
}
