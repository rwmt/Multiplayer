...
public class PingLabelWindow : Window
{
    ...
    public override void DoWindowContents(Rect inRect)
    {
        ...
        using (MpStyle.Set(GameFont.Medium).Set(TextAnchor.MiddleLeft))
            Widgets.Label(new Rect(inRect.x, inRect.y, inRect.width - 30f, TitleH), MpTranslate.Fallback("MpPingLabel_Title", "Rename marker"));
        ...
        if (Widgets.ButtonText(new Rect(inRect.x, btnY, btnW, ButtonH), MpTranslate.Fallback("MpPingLabel_Cancel", "Cancel")))
        {
            Event.current.Use();
            Close();
            return;
        ...
        if (Widgets.ButtonText(new Rect(inRect.x + btnW + Pad, btnY, btnW, ButtonH), MpTranslate.Fallback("MpPingLabel_Confirm", "OK")) || enterPressed)
        {
    ...
    private void Confirm()
    {
        ...
        {
            Messages.Message(MpTranslate.Fallback("MpPingLabel_EmptyReject",
        ...
        {
            Messages.Message(MpTranslate.Fallback("MpPingLabel_NoLongerOwned",
                "You can no longer rename this marker."),
                MessageTypeDefOf.RejectInput, historical: false);
            Close();
            return;
        }
