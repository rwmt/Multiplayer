...
namespace Multiplayer.Client
{
    ...
    public class PingMenuWindow : Window
    {
        ...
        public override void PostOpen()
        {
            ...
            if (string.IsNullOrEmpty(loc.lastUsedCategoryDefName)) return;
            // Mod could have been removed since the last open; silent-fail leaves the wheel idle.
            var cat = DefDatabase<MultiplayerPingDef>.GetNamedSilentFail(loc.lastUsedCategoryDefName);
        ...
        public override void DoWindowContents(Rect inRect)
        {
            ...
            using (MpStyle.Set(GameFont.Medium).Set(TextAnchor.MiddleLeft))
                Widgets.Label(titleRect, "MpPingMenuWindow_Header".Translate());

            var filtersBtnRect = new Rect(titleRect.xMax + HeaderBtnGap, inRect.y + 2f,
                FiltersBtnW, 24f);
            if (Widgets.ButtonText(filtersBtnRect, "MpPingFilters_OpenBtn".Translate()))
            {
            ...
            if (isHost)
            {
                var hostBtnRect = new Rect(filtersBtnRect.xMax + HeaderBtnGap, inRect.y + 2f,
                    HostBtnW, 24f);
                if (Widgets.ButtonText(hostBtnRect, "MpPingHostSettings_OpenBtn".Translate()))
                {
                    var anchor = new Rect(
        ...
        private void DrawContentSection(Rect section)
        {
            ...
            using (MpStyle.Set(GameFont.Tiny).Set(TextAnchor.MiddleLeft).Set(new Color(0.65f, 0.65f, 0.65f)))
                Widgets.Label(headerRect, "MpPingMenuWindow_PlacementType".Translate());
            ...
            using (MpStyle.Set(GameFont.Tiny).Set(TextAnchor.MiddleLeft).Set(WordWrap.NoWrap).Set(new Color(0.78f, 0.78f, 0.78f)))
                Widgets.Label(modeDescRect, ("MpPingMode_" + Multiplayer.settings.pingPlaceMode + "_Description").Translate());
            ...
            using (MpStyle.Set(GameFont.Small).Set(TextAnchor.MiddleLeft))
                Widgets.Label(listHeaderRect, "MpPingMenuWindow_ListHeaderWithCount".Translate(markerCount, cap));
        ...
        private void DrawActionStack(Rect section, ref float y)
        {
            ...
            if (Widgets.ButtonText(clearMineRect, "MpPingDrawer_ClearAllMine".Translate(myCount), active: myCount > 0))
            {
            ...
            if (Widgets.ButtonText(clearOnMapRect, "MpPingDrawer_ClearMineOnThisMap".Translate(onMapCount), active: hasMap && onMapCount > 0))
            {
        ...
        private void DrawList(Rect outRect)
        {
            ...
            if (rows.Count == 0)
            {
                using (MpStyle.Set(GameFont.Small).Set(TextAnchor.MiddleCenter).Set(new Color(0.7f, 0.7f, 0.7f)))
                    Widgets.Label(new Rect(0f, 8f, viewRect.width, 32f), "MpPingMenuWindow_Empty".Translate());
            }
            else
            {
        ...
        private void DrawListRow(Rect rect, PingInfo info)
        {
            ...
            var iconTex = info.category?.IconTexture;
            if (iconTex != null)
            ...
            var catName = info.category?.DisplayName() ?? "";
        ...
        {
            if (info.mapId == -1)
                return "MpPingMenuWindow_TargetPlanet".Translate();
            var map = Find.Maps.GetById(info.mapId);
            return map?.Parent?.LabelCap
                ?? "MpPingMenuWindow_TargetMap".Translate();
        }
        ...
        private static string DeleteLabel()
            => "MpPingMenuWindow_Delete".Translate();
        private static string RenameLabel()
            => "MpPingSel_Rename".Translate();
        ...
        private static string SubtitleLabel()
        {
            ...
            if (loc?.armedCategory is { } cat)
            {
                ...
                return "MpPingMenuWindow_Subtitle_Armed".Translate(catName, modeWord);
            }
            return "MpPingMenuWindow_Subtitle_Idle".Translate(modeWord);
        }

        private static string ModeWordLower(PingPlaceMode m)
            => ("MpPingMode_" + m + "_LowerWord").Translate();

        private static string ModeLabel(PingPlaceMode m)
            => ("MpPingMode_" + m).Translate();
    }
}
